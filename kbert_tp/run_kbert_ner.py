...
def main():
    ...
    def convert_id_to_entity(row):
        row['ner_tags'] = [entity_dict[x] for x in row['ner_tags']]
        return row
    ...
    begin_ids = []
    # Find tagging labels
    dataset_train_initial = load_dataset("jnlpba",split='train')

    dataset_train = dataset_train_initial.map(convert_id_to_entity)
    for line in dataset_train:
        labels = map(str,line['ner_tags'])
        for l in labels:
    ...
    traintest= dataset_train.train_test_split(test_size=0.2, seed=42)
    # Datset loader.
    ...
    def read_dataset(path):
        ...
        if path == "train":
            dataset = traintest['train']
        elif path == "validation":
            dataset = load_dataset("jnlpba",split='validation')
            dataset = dataset.map(convert_id_to_entity)
        elif path == "test":
            dataset = traintest['test']

        print("length of dataset " +" "+str(len(dataset)))
        for line in dataset:
            tokens = line['tokens']
            labels =map(str,line['ner_tags'])
            tokens, pos, vm, tag = kg.add_knowledge_with_vm(tokens, add_pad=True, max_length=args.seq_length)
            tokens = tokens[0]
            pos = pos[0]
            vm = vm[0].astype("bool")
            tag = tag[0]
            ...
            return_dataset.append([tokens, new_labels, mask, pos, vm, tag])
